...
;.ESPERA_FADE_OUT_FINAL_BOSS:
		...
		halt
		djnz	FADE_OUT_FINAL_BOSS_COMUN.ESPERA_FADE_OUT_FINAL_BOSS
		pop		de
		pop		hl
		dec		e
		jr		nz,FADE_OUT_FINAL_BOSS_COMUN.BUCLE_FADE_OUT_FINAL_BOSS
...
;ESPERA_FIN_MUSICA_BOSS_COMUN:
		...
		ld		a,(busply)
		or		a
		jr		z,ESPERA_FIN_MUSICA_BOSS_COMUN.MUSICA_BOSS_TERMINADA
		halt
...
;BUCLE_PINTA_TILES:
		...
		call    DCOMPR_RAM     
		jp		c,BUCLE_PINTA_TILES.SCROLL_DEL_CAMINO
...
.sin_carry:
		...
		jp		nz,BUCLE_PINTA_TILES.SCROLL_DEL_CAMINO									; Si no ha acabado la linea seguiremos pintando tiles
		...
		ld		a,(TRAMO_FASE_3)
		cp		2
		jp		nc,BUCLE_PINTA_TILES.AVISAMO_FINAL_SCROLL
		inc		a
		ld		(TRAMO_FASE_3),a
		add		63
...
;.SCROLL_DEL_CAMINO:
		...
		call    DCOMPR_RAM     
		jp		c,BUCLE_PINTA_TILES.AVANZAMOS_LOS_CONTROLADORES
		...
		ld		a,(Y_DEPH)
		cp		b
		jp		z,BUCLE_PINTA_TILES.RECTIFICA_CONTROL_Y
...
;.SIGUE:
		...
		ld		(CAMBIO_POSE),a
		or		a
		jp		nz,BUCLE_PINTA_TILES.AVANZAMOS_LOS_CONTROLADORES
		...
		cp		1
		jp		z,BUCLE_PINTA_TILES.POSE1
		cp		3
		jp		z,BUCLE_PINTA_TILES.POSE3
...
;.VELOCIDAD_DE_FASE_GALOPE:
		...
		call	PAGE_10_A_SEGMENT_2
		xor		a
		call	BUCLE_PINTA_TILES.APLICAMOS_VELOCIDAD
...
;.bucle_suma:
		...
		adc		hl,de
		djnz	BUCLE_PINTA_TILES.bucle_suma
		...
		call	BUCLE_PINTA_TILES.SACA_S
		call	BUCLE_PINTA_TILES.SACA_T
		call	BUCLE_PINTA_TILES.SACA_A
		call	BUCLE_PINTA_TILES.SACA_G
		call	BUCLE_PINTA_TILES.SACA_E
		jp		BUCLE_PINTA_TILES.SACA_NUM_FASE
...
;.bucle_pinta_prota_frente:
		...
		add		ix,de
		add		4
		djnz	BUCLE_PINTA_TILES.bucle_pinta_prota_frente
...
;.bucle_pinta_saludo:
		...
		ld		a,20
		call	BUCLE_PINTA_TILES.rutina_de_pausa
		djnz	BUCLE_PINTA_TILES.bucle_pinta_saludo
...
;.musica_mas_velocidad:
		...
		jp		BUCLE_PINTA_TILES.VELOCIDAD_DE_FASE_TROTE
...
;.bucle_halt:
		...
		ei
		halt
		djnz	BUCLE_PINTA_TILES.bucle_halt
		pop		bc
		ret
...
;.SALTO_PARA_OTROS_CARTELES:
		...
		or		a
		jp		z,CHECK_POINT.salimos_sin_avisos
...
;PINTA_SPRITE_DEPH:
	...
	[3]	srl		a
		and		00000001B
		or		a
		jp		z,PINTA_SPRITE_DEPH.pintamos_sprite_normal

;.pintamos_sprite_transparente:

		ld		a,(DONDE_VA_LA_INTERRUPCION_LINEAL)
		add		20
		jp		PINTA_SPRITE_DEPH.mismos_datos_para_los_dos
...
;.PATRONES:
		...
		add		4
		add		ix,de
		djnz	PINTA_SPRITE_DEPH.PATRONES

		ld		a,(VARIABLE_CARGA_AGUA)
		or		a
		jp		z,PINTA_SPRITE_DEPH.MAS_COLORES

		ld		hl,COLOR_PIES_EN_LODO
		jp		PINTA_SPRITE_DEPH.SIGUE_TRAS_COLORES_AGUJERO

;.MAS_COLORES:

		ld		a,(SPRITE_CAIDO)
		or		a
		jp		z,PINTA_SPRITE_DEPH.COLORES_NORMALES

		ld		a,(FOTOGRAMA_DEPH_EN_ORDEN)
		cp		2
		jp		z,PINTA_SPRITE_DEPH.COLORES_AGUJERO_POSE_1
		cp		0
		jp		z,PINTA_SPRITE_DEPH.COLORES_AGUJERO_POSE_3
...
;.COLORES_NORMALES:
		...
		ld		a,(FOTOGRAMA_DEPH_EN_ORDEN)
		cp		2
		jp		z,PINTA_SPRITE_DEPH.COLORES_POSE_1
		cp		0
		jp		z,PINTA_SPRITE_DEPH.COLORES_POSE_3
		
;.COLORES_POSE_0_2:

		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		z,PINTA_SPRITE_DEPH.COLORES_POSE_0_2_CON_CASCOS

;.COLORES_POSE_0_2_SIN_CASCOS:		
		
		ld		hl,COLOR_POSE_0_Y_2
		jp		PINTA_SPRITE_DEPH.SIGUE_TRAS_COLORES
...
;.COLORES_POSE_1:
		...
		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		z,PINTA_SPRITE_DEPH.COLORES_POSE_1_CON_CASCOS
...
;.COLORES_POSE_3:
		...
		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		z,PINTA_SPRITE_DEPH.COLORES_POSE_3_CON_CASCOS
...
;.SIGUE_TRAS_COLORES:
		...
		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		nz,PINTA_SPRITE_DEPH.cabeza_normal


;.cabeza_cascos:

		ld		hl,COLOR_DEPH_CASCOS
		jp		PINTA_SPRITE_DEPH.final_color_cabeza
...
;.activamos_sprites:
		...
		or		a
		jp		z,INTERRUPCION_DE_VBLANK.adjust_adecuado
...
;.adjust_aleatorio_2:
		...
		ld		a,r
		and		00000111B
		jp		INTERRUPCION_DE_VBLANK.adjust_conjunto
...
;PALETA_ESCOGIDA:
		...
		ld		a,(ESTADO_COLOR_PERM)
        ld      de,PALETA_ESCOGIDA.TABLA_DE_PALETA_SEGUN_FASE
...
;.A_PALETA_1:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_1												; Cambiamos la p??gina del bloque 2													; Cambiamos la p??gina del bloque 2	
		ld		hl,PALETA_STAGE_1_1
		jp		PALETA_ESCOGIDA.COMUN_PALETAS_2
...
;.A_PALETA_2:
		...
		or		a
		jp		nz,PALETA_ESCOGIDA.A_PALETA_2_2
...
;.A_PALETA_2_2:		
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_5
		jp		PALETA_ESCOGIDA.COMUN_PALETAS_2
...
;.A_FADE_IN_PALETA_1:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_3
		jp		nz,PALETA_ESCOGIDA.A_FADE_IN_PALETA_1_2

		call	PALETA_ESCOGIDA.COMUN_PALETAS_4
		jp		nz,PALETA_ESCOGIDA.A_FADE_IN_PALETA_1_2
...
;.A_FADE_IN_PALETA_2:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_3
		jp		nz,PALETA_ESCOGIDA.A_FADE_IN_PALETA_2_2
...
;.A_FADE_OUT_GRIS:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_3
		jp		nz,PALETA_ESCOGIDA.A_FADE_OUT_GRIS_2
...
;.A_FADE_GRIS_A_BLANCO:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_3
		jp		nz,PALETA_ESCOGIDA.A_FADE_OUT_GRIS_A_BLANCO_2
...
;.A_FADE_BLANCO_A_NEGRO:
		...
		call	PALETA_ESCOGIDA.COMUN_PALETAS_3
		jp		nz,PALETA_ESCOGIDA.A_FADE_OUT_BLANCO_A_NEGRO_2
...
;.posibles_fx:
		...
		ld		a,(FX_ON_OFF)
		or		a
		jp		z,PALETA_ESCOGIDA.ahora_la_musica
...
;.escoge_marcador:
		...
		add		b
		cp		65
		jp		nc,INTERRUPCION_DE_LINEA.segundo_marcador
...
;EFECTOS_ON_OFF:
		...
		ld		a,(FX_ON_OFF)
		or		a
		jp		z,EFECTOS_ON_OFF.encendemos
...
;.bucle_de_arriba:
		...
		add		4
		add		ix,de
		djnz	PAUSE.bucle_de_arriba

		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		nz,PAUSE.color_arriba_normal

;.color_arriba_cascos:

		ld		hl,COLOR_EJERCICIO_1_CASCOS
		jp		PAUSE.ejecuta_el_color
...
;CAMBIAMOS_LA_INTERRUPCION_DE_LINEA_PARA_DESAPARECER:
		...
		ld		a,(HACIA_DONDE_INTERRUPT)
		cp		1
		jp		z,CAMBIAMOS_LA_INTERRUPCION_DE_LINEA_PARA_DESAPARECER.subiendo
		cp		3
		jp		z,CAMBIAMOS_LA_INTERRUPCION_DE_LINEA_PARA_DESAPARECER.bajando
		ret
...
;.bajando2:
		...
		ld		a,159
		jp		CAMBIAMOS_LA_INTERRUPCION_DE_LINEA_PARA_DESAPARECER.fin
...
;.fin:
		...
		ld		a,(HACIA_DONDE_INTERRUPT)
		cp		1
		jp		z,CAMBIAMOS_LA_INTERRUPCION_DE_LINEA_PARA_DESAPARECER.revisa_si_desaparece
...
;.bucle_5_ceros:
		...
		add		ix,de
		djnz	MUERTE_POR_TOQUES.bucle_5_ceros
		ld		ix,ATRIBUTOS_DEPH_VARIABLES
...
;.bucle_5_datos:
		...
		add		4
		add		ix,de
		djnz	MUERTE_POR_TOQUES.bucle_5_datos
		ld		ix,ATRIBUTOS_DEPH_VARIABLES
...
;.bucle_para_borrar_sprites_activos:
		...
		ld		(ix),a
		add		ix,de
		djnz	BORRA_SPRITES_ACTIVOS.bucle_para_borrar_sprites_activos
...
;VUELCA_DATOS_COLORES_DEPH_A_VRAM:
		...
		ld		a,(MUSICA_ON_OFF)
		or		a
		jp		nz,VUELCA_DATOS_COLORES_DEPH_A_VRAM.color_normal
